...
from utils.sql_helper import get_all_projects, create_prompt, get_all_prompts, get_prompt_details
...
if 'ALL_PROMPTS' not in st.session_state:
    st.session_state['ALL_PROMPTS'] = get_all_prompts(cursor, st.session_state['CURRENT_PROJECT_ID'])
    
    prompt_list = []
    for id in st.session_state['ALL_PROMPTS']:
        prompt_list.append(st.session_state['ALL_PROMPTS'][id])
        prompt_list[-1]['id'] = id
    
    st.session_state['ALL_PROMPTS'] = pd.DataFrame(prompt_list)
...
if 'OLD_PROMPT_INFO' not in st.session_state:
    st.session_state['OLD_PROMPT_INFO'] = None
...
with subheader_cols[1]:
    ...
    subheader_cols = st.columns([.4, .2, .4])
    
    subheader_cols[0].selectbox('Prompt Name:', options=['None'] + st.session_state['ALL_PROMPTS']['name'].unique().tolist(), key='prompt_name_selectbox')
    ...
    subheader_cols[2].selectbox('Version:', options=version_options, key='version_selectbox')
    
    st.columns(3)[1].button('Load', key='load_button', use_container_width=True, disabled=st.session_state['prompt_name_selectbox'] == 'None' or st.session_state['version_selectbox'] == 'None')
    
    if st.session_state['load_button']:
        st.session_state['OLD_PROMPT_INFO'] = get_prompt_details(cursor, st.session_state['PROMPT_ID_LOOKUP'][(st.session_state['prompt_name_selectbox'], st.session_state['version_selectbox'])])
        st.rerun()
...
if st.session_state['OLD_PROMPT_INFO'] is not None:
    ...
    if st.session_state['commit_button']:
        _, prompt_group_id = create_prompt(
        ...
        st.session_state['CURRENT_PROMPT_GROUP_ID'] = prompt_group_id
        sleep(1)
        clean_session_state()
