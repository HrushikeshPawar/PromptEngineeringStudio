...
from utils.sql_helper import get_all_projects, update_prompt, get_prompt_details
...
if 'CURRENT_PROMPT_INFO' not in st.session_state:
    st.session_state['CURRENT_PROMPT_INFO'] = get_prompt_details(cursor, st.session_state['CURRENT_PROMPT_ID'])
...
with main_columns[3]:
    ...
    st.text_input(
        label='Prompt Name',
        key='prompt_name',
        value=st.session_state['CURRENT_PROMPT_INFO']['name'],
    ...
    st.number_input(
        label='Version',
        value=st.session_state['CURRENT_PROMPT_INFO']['version'],
        key='version',
        label_visibility='collapsed',
        disabled=True,
    )
    ...
    st.text_input(
        label='Description',
        key='description',
        value=st.session_state['CURRENT_PROMPT_INFO']['description'] if st.session_state['CURRENT_PROMPT_INFO']['description'] is not None else '',
    ...
    st.text_input(
        label='Parent',
        key='parent',
        value=st.session_state['CURRENT_PROMPT_INFO']['parent_prompt_id'],
    ...
    # Add to favorites
    st.columns(3)[1].toggle('Add to favorites', key='add_to_favorites', value=st.session_state['CURRENT_PROMPT_INFO']['favourite'])
    ...
    st.text_area(
        ...
        value=st.session_state['CURRENT_PROMPT_INFO']['notes'] if st.session_state['CURRENT_PROMPT_INFO']['notes'] is not None else '',
        height=100,
        label_visibility='collapsed',
    )
...
with main_columns[1]:
    ...
    info_bar_options['info'][0]['name'] = f"Prompt Template Editor | {st.session_state['prompt_name'] if len(st.session_state['prompt_name'].strip()) > 0 else 'Project Name'} | v{st.session_state['version']} |"
    ...
    prompt_template_editor = code_editor(
        code=st.session_state['CURRENT_PROMPT_INFO']['prompt_template'],
        options=options,
        height=[10, 100],
        lang='text',
...
if st.session_state['commit_button']:
    update_prompt(
        id=st.session_state['CURRENT_PROMPT_ID'],
    ...
    st.success('Current prompt version updated successfully!')
    sleep(1)
    clean_session_state()
